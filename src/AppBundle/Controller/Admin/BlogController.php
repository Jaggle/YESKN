...
class BlogController extends Controller
{
    ...
    public function newAction(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $tag = new Tag();
            $tag->setName($request->get('_post_tag'));
            $tag->setAlias($request->get('_post_tag'));
            $tag->setStatus(1);
            $tagEntityManager = $this->getDoctrine()->getManager();
            $tagEntityManager->persist($tag);
            $tagEntityManager->flush();
    ...
    public function showAction(Post $post)
    {
        ...
        $tag = $em->getRepository('AppBundle:Tag')->find($post->getTagId() ?: 0);
        $tagName = $tag ? $tag->getName() : '';
